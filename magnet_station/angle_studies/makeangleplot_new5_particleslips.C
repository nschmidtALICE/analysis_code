...
void makeangleplot_new5_particleslips(TString inputfile = "/home/niviths/Downloads/magnetStationSims/minimumBias_MS_MagDown_2000plus.root") // tilt
{
    ...
    TH2F *hYZHitsinPlane = new TH2F("hYZHitsinPlane", "YZ of hits in plane", 125, minZ, maxZ, 100, -1500, 1500);
    TH2F *hEvsEtaHitsinPlane = new TH2F("hEvsEtaHitsinPlane", "E vs Eta of hits in plane", 100, 1.5, 4.5, 100, 0, 5000);
    ...
    int maxbar = 0;
    bool onceMsg = 1;
    int npartwithhitinplane = 0;
    int npartwithouthitinplane = 0;
    while (tree.Next())
    {
        ...
        for (auto i = 0U; i < pt.GetSize(); ++i)
        {
            ...
            int nfthits = 0;
            int nhitsinplane = 0;
            float xpos_hitinPlane = -50000;
            float ypos_hitinPlane = -50000;
            float zpos_hitinPlane = -50000;
            ...
            for (auto j = 0U; j < ms_px.GetSize(); ++j)
            {
                ...
                // TODO REMOVE THIS LINE BELOW!!!
                if (abs(ms_vy[j]) < 150 || abs(ms_vy[j]) > 900 || ms_vz[j] < 5000 || ms_vz[j] > 7000)
                {
                    continue;
                }
                ...
                // cout << bar << "\t" << (ms_bitID[j] >> 22) <<  endl;
                // cout << isfiber << "\t" << (ms_bitID[j] >> 30) <<  endl;
                // if(isfiber) cout << "Fiber hit" << endl;
                ...
                {
                    nhitsinplane++;
                    xpos_hitinPlane = ms_vx[j];
                    ypos_hitinPlane = ms_vy[j];
                    zpos_hitinPlane = ms_vz[j];
                }
                nHitsPartSum += 1;
            ...
            // if(nhitsinplane>0) cout << "nhitsinplane: " << nhitsinplane << " xpos: " << xpos_hitinPlane << " ypos: " << ypos_hitinPlane << " zpos: " << zpos_hitinPlane << endl;
            if (nhitsinplane > 0 && nHitsMS == 0)
            {
                hYZHitsinPlane->Fill(zpos_hitinPlane, ypos_hitinPlane);
                hEvsEtaHitsinPlane->Fill(eta[i], p[i]);
            }
            ...
            if (nhitsinplane > 0)
                npartwithhitinplane++;
            if (nHitsMS > 0 && nhitsinplane == 0)
                npartwithouthitinplane++;
        } // NOTE END LOOP OVER PARTICLES
        // print number of particles with hits in plane and without hits in plane as well as ratio
    ...
    cout << "Particles with hits in plane: " << npartwithhitinplane << " Particles without hits in plane: " << npartwithouthitinplane << " Ratio: " << (float)npartwithhitinplane / (float)npartwithouthitinplane << endl;
    ...
    // plot hYZHitsinPlane
    TCanvas *c1_hYZHitsinPlane = new TCanvas("c1_hYZHitsinPlane", "c1_hYZHitsinPlane", 800, 600);
    hYZHitsinPlane->GetXaxis()->SetTitle("Z [mm]");
    hYZHitsinPlane->GetYaxis()->SetTitle("Y [mm]");
    hYZHitsinPlane->Draw("colz");
    c1_hYZHitsinPlane->SaveAs("hYZHitsinPlane.pdf");

    // plot hEvsEtaHitsinPlane
    TCanvas *c1_hEvsEtaHitsinPlane = new TCanvas("c1_hEvsEtaHitsinPlane", "c1_hEvsEtaHitsinPlane", 800, 600);
    hEvsEtaHitsinPlane->GetXaxis()->SetTitle("#eta");
    hEvsEtaHitsinPlane->GetYaxis()->SetTitle("p [MeV]");
    hEvsEtaHitsinPlane->Draw("colz");
    c1_hEvsEtaHitsinPlane->SaveAs("hEvsEtaHitsinPlane.pdf");
