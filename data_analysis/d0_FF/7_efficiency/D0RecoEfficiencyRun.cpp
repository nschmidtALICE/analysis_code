...
D0RecoEfficiency::D0RecoEfficiency(TString inputFileName,
      ...
      m_minDaughterMomentum(2.0)
{
    ...
    // Initialize MC truth daughter variables
    mc_dau_pid = nullptr;
    mc_dau_matched = nullptr;
...
D0RecoEfficiency::~D0RecoEfficiency()
{
    CleanUp();
}

void D0RecoEfficiency::CleanUp()
{
...
bool D0RecoEfficiency::Initialize()
{
    ...
    std::cout << "D0RecoEfficiency initialized successfully" << std::endl;
...
void D0RecoEfficiency::InitializeBranches()
{
    ...
    // Note: Some branches may be optional
    ...
    // Set branch addresses for MC truth daughter variables (optional)
...
void D0RecoEfficiency::CreateHistograms()
{
...
void D0RecoEfficiency::CalculateRecoEfficiency()
{
    std::cout << "\nCalculating reconstruction efficiency..." << std::endl;
    ...
    // Pre-calculate acceptance limits for faster comparison
    const double minPt = m_minPt;
    const double minEta = m_minEta;
    const double maxEta = m_maxEta;
    ...
    std::cout << "\nStatistics:" << std::endl;
    std::cout << "  Total MC D0s in acceptance: " << totalDen << std::endl;
    std::cout << "  Total reconstructed D0s passing selection: " << totalNum << std::endl;
    std::cout << "  Overall reconstruction efficiency: " << overallEff * 100.0 << "% (" << totalNum << "/" << totalDen << ")" << std::endl;
}

bool D0RecoEfficiency::PassesD0Selection(int d0_idx)
{
    ...
    // Apply selection cuts (fastest cuts first)
    if (pt < m_minPt)
        return false;
    if (eta > m_maxEta || eta < m_minEta)
        return false;
    if (TMath::Abs(mass - d0_pdg_mass) > m_d0MassWindow * 0.001)
        return false; // Convert MeV to GeV

    // Apply quality cuts using MC truth PID (not reconstructed PID)
    if (!PassesQualityCutsOptimized(d0_idx))
        return false;
    // Note: PID selection is now done using MC truth in PassesQualityCutsOptimized

    return true;
}

bool D0RecoEfficiency::PassesPIDSelection(int d0_idx)
{
    ...
    if (kaon_idx < 0 || pion_idx < 0)
        return false;

    // Check PID cuts
    // if (dau_pnn_k->at(kaon_idx) < m_kaonPIDCut)
    //     return false;
    // if (dau_pnn_pi->at(pion_idx) < m_pionPIDCut)
    //     return false;

    return true;
}

bool D0RecoEfficiency::PassesQualityCutsOptimized(int d0_idx)
{
    ...
    for (size_t dau_idx = 0; dau_idx < dau_d0_idx->size(); ++dau_idx)
    {
        ...
        // Check quality cuts first (fastest to fail)
        // if (dau_prb_ghost->at(dau_idx) > m_ghostProbCut)
        //     return false;
        // if (dau_trckChi2->at(dau_idx) > m_trackChi2Cut)
        //     return false;
        ...
        if (mc_dau_pid && dau_idx < mc_dau_pid->size()) {
            ...
            // std::cout << "Daughter " << dau_idx << ": MC PID = " << mc_pid << std::endl;
        } else {
            // std::cout << "Warning: mc_dau_pid not available" << std::endl;
        }
        // else {
        //     // Fallback to reconstructed PID if MC truth not available
        //     int pid = dau_pid->at(dau_idx);
        //     if (abs(pid) == 321) { // Kaon
        //         foundTrueKaon = true;
        //     }
...
double D0RecoEfficiency::CalculateMCMomentum(int mc_idx)
{
...
void D0RecoEfficiency::SaveResults()
{
    if (!m_outputFile)
    {
...
void D0RecoEfficiency::PlotEfficiency(const std::string &histName)
{
...
int D0RecoEfficiencyRun(TString inputFile = "/media/niviths/SSD2/lhcb_analysis_SSD/20250708_newMC_fixedTrueAssociation/51/51.root", TString outputFile = "output.root",
                        ...
                        bool makePlots = true)
{
    ...
    std::cout << "=== D0 Reconstruction Efficiency Calculator ===" << std::endl;
    ...
    try
    {
        ...
        D0RecoEfficiency calculator(inputFile, outputFile);
        ...
        // Set up custom binning (twice as fine as previous)
