...
void analyze_Lc(bool makeTree = false)
{
    // // Open the .root file
    ...
    // tree->Add("/media/niviths/SSD2/lhcb_analysis_SSD/20250324_LcD0AP/1_LcpKpi_DecayFunTuple.root");
    // tree->Add("/media/niviths/SSD2/lhcb_analysis_SSD/20250324_LcD0AP/2_LcpKpi_DecayFunTuple.root");
    // tree->Add("/media/niviths/SSD2/lhcb_analysis_SSD/20250324_LcD0AP/3_LcpKpi_DecayFunTuple.root");
    // tree->Add("/media/niviths/SSD2/lhcb_analysis_SSD/20250324_LcD0AP/4_LcpKpi_DecayFunTuple.root");
    // tree->Add("/media/niviths/SSD2/lhcb_analysis_SSD/20250324_LcD0AP/5_LcpKpi_DecayFunTuple.root");
    // tree->Add("/media/niviths/SSD2/lhcb_analysis_SSD/20250324_LcD0AP/6_LcpKpi_DecayFunTuple.root");
    // tree->Add("/media/niviths/SSD2/lhcb_analysis_SSD/20250324_LcD0AP/7_LcpKpi_DecayFunTuple.root");
    // tree->Add("/media/niviths/SSD2/lhcb_analysis_SSD/20250324_LcD0AP/8_LcpKpi_DecayFunTuple.root");
    // tree->Add("/media/niviths/SSD2/lhcb_analysis_SSD/20250324_LcD0AP/9_LcpKpi_DecayFunTuple.root");
    // if (!tree) {
    //     std::cerr << "Error getting tree from file" << std::endl;
    //     file->Close();
    //     return;
    ...
    Double_t Lc_DOCA;
    Double_t Lc_CHI2DOF;
    Float_t Lc_BPVFDCHI2, Lc_BPVIPCHI2;
    Float_t Lc_B_PV_Z, Lc_PZ, Lc_END_VZ, Lc_BPVLTIME;
    ...
    tree->SetBranchAddress("Lc_BPVIPCHI2", &Lc_BPVIPCHI2);
    tree->SetBranchAddress("Lc_B_PV_Z", &Lc_B_PV_Z);
    tree->SetBranchAddress("Lc_PZ", &Lc_PZ);
    tree->SetBranchAddress("Lc_END_VZ", &Lc_END_VZ);
    tree->SetBranchAddress("Lc_BPVLTIME", &Lc_BPVLTIME);
    ...
    if (makeTree)
    {
        ...
        outTree->Branch("Lc_Tz", &Lc_Tz, "Lc_Tz/D");
        outTree->Branch("Lc_Y", &Lc_Y, "Lc_Y/F");
        ...
        outTree->Branch("Lc_BPVIPCHI2", &Lc_BPVIPCHI2, "Lc_BPVIPCHI2/F");
        outTree->Branch("Lc_B_PV_Z", &Lc_B_PV_Z, "Lc_B_PV_Z/F");
        outTree->Branch("Lc_PZ", &Lc_PZ, "Lc_PZ/F");
        outTree->Branch("Lc_END_VZ", &Lc_END_VZ, "Lc_END_VZ/F");
        outTree->Branch("Lc_BPVLTIME", &Lc_BPVLTIME, "Lc_BPVLTIME/F");
    ...
    TH1F *h_Lc_MASS_pAr = new TH1F("h_Lc_MASS_pAr", "h_Lc_MASS_pAr", 120, 2287 - 60, 2287 + 60);
    TH1F *h_Lc_MASS_pH = new TH1F("h_Lc_MASS_pH", "h_Lc_MASS_pH", 120, 2287 - 60, 2287 + 60);
    TH1F *h_Lc_MASS_pNe = new TH1F("h_Lc_MASS_pNe", "h_Lc_MASS_pNe", 120, 2287 - 60, 2287 + 60);
    ...
    // make log(Lc_BPVIPCHI2) histogram
    TH1F *h_Lc_BPVIPCHI2_pAr = new TH1F("h_Lc_BPVIPCHI2_pAr", "h_Lc_BPVIPCHI2_pAr", 150, -3, 6);
    TH1F *h_Lc_BPVIPCHI2_pH = new TH1F("h_Lc_BPVIPCHI2_pH", "h_Lc_BPVIPCHI2_pH", 150, -3, 6);
    TH1F *h_Lc_BPVIPCHI2_pNe = new TH1F("h_Lc_BPVIPCHI2_pNe", "h_Lc_BPVIPCHI2_pNe", 150, -3, 6);
    ...
    for (Long64_t i = 0; i < nEntries; ++i)
    {
        ...
        //  acceptance cuts
        if (Km_PT < 400 || pip_PT < 400 || pp_PT < 400)
        {
            h_cutFlow->Fill(currcut);
            continue;
        }
        h_nEvents_cutFlow->Fill(currcut);
        currcut++; // 2
        if (Km_ETA < 2 || Km_ETA > 5 || pip_ETA < 2 || pip_ETA > 5 || pp_ETA < 2 || pp_ETA > 5)
        {
            h_cutFlow->Fill(currcut);
            continue;
        }
        h_nEvents_cutFlow->Fill(currcut);
        currcut++; // 3
        if (Km_P / 1000 > 100 || Km_P / 1000 < 3.2 || pip_P / 1000 > 100 || pip_P / 1000 < 3.2 || pp_P / 1000 > 100 || pp_P / 1000 < 3.2)
        {
            h_cutFlow->Fill(currcut);
            continue;
        }
        ...
        currcut++; // 4
        // quality cuts
        if (Km_CHI2DOF > 3 || pip_CHI2DOF > 3 || pp_CHI2DOF > 3)
        {
            h_cutFlow->Fill(currcut);
            continue;
        }
        ...
        if (Km_PROBNNGHOST > 0.3 || pip_PROBNNGHOST > 0.3 || pp_PROBNNGHOST > 0.3)
        {
            h_cutFlow->Fill(currcut);
            continue;
        }
        ...
        if (Km_BPVIPCHI2 < 16 || pip_BPVIPCHI2 < 16 || pp_BPVIPCHI2 < 16)
        {
            h_cutFlow->Fill(currcut);
            continue;
        }
        ...
        // PID cuts
        if (Km_PIDK < 5 || pip_PIDK > 0 || pp_PIDP < 15 || (pp_PIDP - pp_PIDK) < 15)
        {
            h_cutFlow->Fill(currcut);
            continue;
        }
        ...
        if (Lc_BPVDIRA < 0.99974689 /*TMath::Cos(0.0225)*/)
        {
            h_cutFlow->Fill(currcut);
            continue;
        }
        ...
        double t_z = (Lc_END_VZ - Lc_B_PV_Z) * Lc_MASS / Lc_PZ;
        Lc_Tz = t_z;
        tz = t_z;
        tzError = t_z / 100;
        mass = Lc_MASS;
        ...
        { // pAr
            h_Lc_MASS_pAr->Fill(Lc_MASS);
            h_Lc_BPVIPCHI2_pAr->Fill(log10(Lc_BPVIPCHI2));
        }
        if (SMOGInjectedGas == 1)
        { // pH
            h_Lc_MASS_pH->Fill(Lc_MASS);
            h_Lc_BPVIPCHI2_pH->Fill(log10(Lc_BPVIPCHI2));
        }
        if (SMOGInjectedGas == 6)
        { // pNe
            h_Lc_MASS_pNe->Fill(Lc_MASS);
            h_Lc_BPVIPCHI2_pNe->Fill(log10(Lc_BPVIPCHI2));
        }
    }
    ...
    double max = h_Lc_MASS_pAr->GetMaximum();
    if (h_Lc_MASS_pH->GetMaximum() > max)
        max = h_Lc_MASS_pH->GetMaximum();
    if (h_Lc_MASS_pNe->GetMaximum() > max)
        max = h_Lc_MASS_pNe->GetMaximum();
    h_Lc_MASS_pAr->GetYaxis()->SetRangeUser(0, max * 1.1);
    ...
    makeLHCbStyleMassPlot(h_Lc_MASS_pAr, outputdir, "pAr", "p^{+}K^{-}#pi^{+}", "#Lambda_{c}", "Lc");
    makeLHCbStyleMassPlot(h_Lc_MASS_pH, outputdir, "pH", "p^{+}K^{-}#pi^{+}", "#Lambda_{c}", "Lc");
    makeLHCbStyleMassPlot(h_Lc_MASS_pNe, outputdir, "pNe", "p^{+}K^{-}#pi^{+}", "#Lambda_{c}", "Lc");
    ...
    h_Lc_BPVIPCHI2_pAr->Write();
    h_Lc_BPVIPCHI2_pH->Write();
    h_Lc_BPVIPCHI2_pNe->Write();
    fout->Close();
